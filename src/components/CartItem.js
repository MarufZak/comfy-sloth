...
const CartItem = ({id,color,amount,product}) => {
  const {cart,toggleAmount,removeItem} = useCartContext();

  const increase = ()=>{
    toggleAmount(id,"incr",color)
  }

  const decrease = ()=>{
    toggleAmount(id,"decr",color)
    let el = cart.find(item=>item.id===id&&item.color===color)
    if (el && el.amount-1 === 0) {
      removeItem(id,color);
    }
  }
  ...
  return <Wrapper>
    ...
    <h5 className="subtotal">{formatPrice(product.price * amount)}</h5>
    <div onClick={()=>removeItem(id,color)} className="remove-btn">
      <FaTrash/>
    </div>
  </Wrapper>
}
